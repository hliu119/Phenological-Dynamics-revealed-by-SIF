...
for i in range(len(site_pd)):
    ...
    file = os.path.join(inroot, "Output_VIs\{0}\{1}.csv".format(region, site_name))
    ...
    for j in range(len(dir_pd)):
        ...
        tt = dt.timetuple()
        if not np.isnan(qa_pd.iloc[j,1]):
            if int(qa_pd.iloc[j,1]) == -1 or int(qa_pd.iloc[j,1]) == 2 or int(qa_pd.iloc[j,1]) == 3:
                if tt.tm_year == 2018:
                    list18.append([tt.tm_yday, np.nan, np.nan])
                if tt.tm_year == 2019:
                    list19.append([tt.tm_yday, np.nan, np.nan]) 
            else:
                 if tt.tm_year == 2018:
    ...
    arr_EVI_Merge = arr_EVI #Merge_Ori(arr_EVI, day_length)
    arr_NIRv_Merge = arr_NIRv #Merge_Ori(arr_NIRv, day_length) 
        
    outPath_EVI = os.path.join(outroot, "Output_EVI\{0}\SitePixel\Polygon".format(region))
    outPath_NIRv= os.path.join(outroot, "Output_NIRv\{0}\SitePixel\Polygon".format(region))
    
    if not os.path.exists(outPath_EVI):
        os.makedirs(outPath_EVI)
    if not os.path.exists(outPath_NIRv):
        os.makedirs(outPath_NIRv)
        
    outfile_EVI = os.path.join(outPath_EVI, "{0}.csv".format(site_name))
    outfile_NIRv= os.path.join(outPath_NIRv,"{0}.csv".format(site_name))
